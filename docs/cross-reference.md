...
| `orchestrator status` | §12 (미상세) | ⚠️ |
| `orchestrator cancel` | §12 (미상세) | ⚠️ |
| `orchestrator resume` | §12 (미상세) | ⚠️ |
| `orchestrator presets list` | §12 (미상세) | ⚠️ |
| `orchestrator presets show` | §12 (미상세) | ⚠️ |

> ⚠️ 표시 항목: run과 serve만 상세 정의됨. 나머지 CLI 명령은 간략하게만. 구현 시 deployment.md 기준으로 보강 필요.
