...
# --- Skills (CLI에 주입할 스킬) ---
skills:                          # list[str], 선택, 활성화할 스킬 목록
  - ""                           # 예: "log-analysis", "metric-query", "k8s-ops"
...
| 필드 | 타입 | 필수 | 기본값 | 설명 |
|------|------|------|--------|------|
| `preferred_cli` | `str` | **예** | `"claude"` | `"claude"`, `"codex"`, `"gemini"` — 사용할 CLI 도구 |
| `fallback_cli` | `str` | 아니오 | `""` | 기본 CLI 실패 시 대체 CLI |
| `model_override` | `str` | 아니오 | `""` | CLI 도구의 LLM 모델 오버라이드 |

#### `mcp_servers`

CLI 에이전트에 주입할 MCP 서버 목록이다. `preferred_cli`에 따라 주입 방식이 다르다:
- **Claude**: `--mcp-config '{"mcpServers":{...}}'` 플래그
- **Codex**: `CODEX_HOME/config.toml`의 `[mcp_servers]` 섹션
- **Gemini**: `.gemini/settings.json`의 `mcpServers` 필드
...
| 타입 | 필수 | 기본값 | 설명 |
|------|------|--------|------|
| `list[str]` | 아니오 | `[]` | 에이전트에 활성화할 스킬 목록. 예: `["log-analysis", "metric-query"]` |
...
### 3.3 `elk-analyst.yaml` — ELK 로그 분석 (Claude CLI + MCP)

```yaml
# presets/agents/elk-analyst.yaml
name: elk-analyst
description: "ELK(Elasticsearch) 기반 로그 분석 에이전트 — Claude CLI + Elasticsearch MCP 서버"
version: "1.0"
...
# MCP 서버는 CLI에 주입된다 (Claude: --mcp-config 플래그)
mcp_servers:
  elasticsearch:
    command: "npx"
    args:
...
### 3.5 `grafana-monitor.yaml` — Grafana 메트릭 분석 (Claude CLI + MCP)

```yaml
# presets/agents/grafana-monitor.yaml
name: grafana-monitor
description: "Grafana 메트릭 모니터링 에이전트 — Claude CLI + Grafana MCP 서버"
version: "1.0"

persona:
  role: "Grafana 메트릭 분석 전문가"
  goal: "Grafana 대시보드 메트릭에서 이상 징후를 식별하고 분석 보고서를 작성한다"
  backstory: |
    SRE 팀에서 Grafana/Prometheus 기반 모니터링 시스템을 6년간 운영한 엔지니어로,
    CPU, 메모리, 네트워크, 응답시간 등 시스템 메트릭을 분석하여
    장애 원인을 빠르게 파악하는 능력이 뛰어나다.
  constraints:
    - "최근 1시간 메트릭을 우선 분석하고, 필요 시 범위를 확장한다"
    - "정상 baseline 대비 이상 수치를 명확히 구분한다"
    - "메트릭 수치와 그래프 트렌드를 함께 보고한다"
    - "상관관계 분석 시 인과관계와 구분한다"
...
prompt:
  system_suffix: |
    메트릭 분석 보고서는 다음 형식으로 작성한다:
    1. 요약 (1-2문장)
    2. 이상 메트릭 목록 (메트릭명, 현재값, 정상 baseline, 편차)
    3. 시간대별 트렌드 분석
    4. 상관관계 분석 (메트릭 간 연관성)
    5. 근본 원인 추정
    6. 권장 조치
...
### 3.6 `k8s-operator.yaml` — Kubernetes 운영 (Gemini CLI + MCP)

```yaml
# presets/agents/k8s-operator.yaml
name: k8s-operator
description: "Kubernetes 클러스터 운영 에이전트 — Gemini CLI + K8s MCP 서버"
version: "1.0"

persona:
  role: "Kubernetes 운영 전문가"
  goal: "Kubernetes 클러스터 상태를 점검하고 인프라 이슈를 진단한다"
  backstory: |
    DevOps/SRE 팀에서 대규모 Kubernetes 클러스터를 4년간 운영한 엔지니어로,
    Pod 스케줄링, 리소스 관리, 네트워크 정책 등에 정통하다.
    장애 발생 시 kubectl 명령과 이벤트 로그로 빠르게 원인을 파악한다.
  constraints:
    - "Pod restart, OOMKilled, CrashLoopBackOff 이벤트에 집중한다"
    - "최근 배포 이력을 반드시 확인한다"
    - "리소스 사용량(CPU/Memory) requests vs limits를 비교한다"
    - "destructive 명령(delete, drain 등)은 권장하지 않는다"
...
prompt:
  system_suffix: |
    Kubernetes 진단 보고서는 다음 형식으로 작성한다:
    1. 요약 (1-2문장)
    2. 클러스터 상태 개요 (노드, Pod 수, 이상 Pod)
    3. 이상 이벤트 목록 (시간순)
    4. 리소스 사용량 분석
    5. 최근 배포 이력과 연관성
    6. 근본 원인 추정
...
### 3.7 `incident-analysis-team.yaml` — 인시던트 분석 팀 (ELK + Grafana + K8s)
...
| `execution.preferred_cli` | CLI 도구 변경은 프리셋 재정의 필요 (MCP 주입 방식이 CLI별로 다름) |
...
### Step 7: CLI + MCP 에이전트 프리셋

MCP 서버를 도구로 사용하는 CLI 에이전트 프리셋 작성:

```yaml
# ~/.config/orchestrator/presets/agents/github-analyst.yaml
name: github-analyst
description: "GitHub 이슈/PR 분석 에이전트 — Claude CLI + GitHub MCP 서버"
...
# MCP 서버는 CLI에 --mcp-config 플래그로 주입된다
mcp_servers:
  github:
    command: "npx"
    args:
...
> **참고:** 모든 에이전트는 CLI를 통해 실행된다. MCP 서버는 CLI의 도구로 주입되며 (`--mcp-config`, `CODEX_HOME`, `.gemini/settings.json`), 별도의 실행 모드가 아니다. 환경변수에서 `${VAR_NAME}` 문법을 사용하면 시스템 환경변수로 치환된다. 프리셋 YAML에 API 키를 직접 작성하지 않는다.
