/**
 * Shared JSONL stream parser for CLI agent output.
 *
 * Used by LiveOutput and LiveOutputInline to parse Claude / Codex / Gemini
 * streaming events into human-readable OutputLine objects.
 */
...
/* ── CLI JSONL event → human-readable format ── */

export function formatClaudeEvent(ev: Record<string, unknown>): OutputLine | null {
...
export function formatCodexEvent(ev: Record<string, unknown>): OutputLine | null {
  const t = ev.type as string;
...
export function formatGeminiEvent(ev: Record<string, unknown>): OutputLine | null {
  const t = ev.type as string;
  if (t === "init") {
...
export function parseStreamLine(rawLine: string): OutputLine | null {
